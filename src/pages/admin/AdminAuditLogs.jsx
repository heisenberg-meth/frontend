...
const ACTION_COLORS = {
  ADMIN_LOGIN: "#3b82f6", ADMIN_LOGOUT: "#6366f1",
  USER_CREATED: "#22c55e", USER_UPDATED: "#22c55e", USER_DELETED: "#ef4444",
  USER_SUSPENDED: "#f59e0b", USER_ACTIVATED: "#22c55e", USER_BLOCKED: "#dc2626",
  SHOP_CREATED: "#22c55e", SHOP_SUSPENDED: "#f59e0b", SHOP_BLACKLISTED: "#dc2626",
  DEVICE_BLOCKED: "#dc2626", DEVICE_UNBLOCKED: "#22c55e",
  SUBSCRIPTION_EXTENDED: "#22c55e", SUBSCRIPTION_CANCELLED: "#ef4444",
        ...
        <table className="admin-table">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={6} className="admin-empty">No audit logs found</td></tr>
            ) : logs.map((log) => (
              <tr key={log.id}>
                <td className="admin-cell-nowrap">{new Date(log.createdAt).toLocaleString()}</td>
                <td>
                  <span className="admin-badge" style={{ background: ACTION_COLORS[log.action] || "#666" }}>
                    {log.action}
                  </span>
                </td>
                <td><small>{log.targetType} / {log.targetId?.slice(0, 8)}</small></td>
                <td><code>{log.adminUserId?.slice(0, 8)}</code></td>
                <td>{log.ipAddress || "—"}</td>
