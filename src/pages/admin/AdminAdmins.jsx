...
const ROLE_COLORS = {
  ROOT_ADMIN: "#ef4444", ADMIN: "#3b82f6", SUPPORT: "#22c55e", SALES: "#f59e0b", FINANCE: "#8b5cf6",
};
...
export default function AdminAdmins() {
  ...
  const [form, setForm] = useState({ email: "", password: "", name: "", role: "SUPPORT", permissions: "" });

  const fetchAdmins = async () => {
    setLoading(true);
    try {
      const res = await adminApi.getAdmins({ page, limit: 20 });
      if (res.success) {
        setAdmins(res.data || []);
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      setShowForm(false);
      setForm({ email: "", password: "", name: "", role: "SUPPORT", permissions: "" });
      fetchAdmins();
    } catch (err) {
        ...
        <table className="admin-table">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={8} className="admin-empty">No admin users found</td></tr>
            ) : admins.map((a) => (
              <tr key={a.id}>
                <td><strong>{a.name}</strong></td>
                <td>{a.email}</td>
                <td>
                  <span className="admin-badge" style={{ background: ROLE_COLORS[a.role] || "#666" }}>
                    {a.role}
                  </span>
                </td>
                ...
                <td>
                  {a.isActive
                    ...
                    : <span className="admin-status admin-status-blocked">INACTIVE</span>}
                </td>
