  ...
  return <div className="admin-page">
      ...
      {showForm && <form className="admin-form" onSubmit={handleCreate}>
          <div className="admin-form-row">
            <><label htmlFor="field_q7r4h1" className="sr-only">Key (e.g. AI_MODULE)</label><input placeholder="Key (e.g. AI_MODULE)" value={form.key} onChange={e => setForm({
            ...form,
            key: e.target.value
          })} required id="field_q7r4h1" /></>
            <><label htmlFor="field_a7hjp3" className="sr-only">Name (e.g. AI Module)</label><input placeholder="Name (e.g. AI Module)" value={form.name} onChange={e => setForm({
            ...form,
            name: e.target.value
          })} required id="field_a7hjp3" /></>
          </div>
          <div className="admin-form-row">
            <><label htmlFor="field_9hoz02" className="sr-only">Description</label><input placeholder="Description" value={form.description} onChange={e => setForm({
            ...form,
          ...
          {form.targetType !== "ALL" && <><label htmlFor="field_vdq7is" className="sr-only">Target IDs (comma-separated)</label><input placeholder="Target IDs (comma-separated)" value={form.targetIds} onChange={e => setForm({
          ...form,
        ...
        <table className="admin-table">
          ...
          <tbody>
            {loading ? <tr>
              ...
              </tr> : flags.length === 0 ? <tr>
                <td colSpan={7} className="admin-empty">
                  No feature flags created yet
                </td>
              </tr> : flags.map(f => <tr key={f.id}>
                  <td>
                    <code>{f.key}</code>
                  </td>
                  <td>
                  ...
                  <td>
                    {f.enabled ? <span className="admin-status admin-status-active">
                        ENABLED
