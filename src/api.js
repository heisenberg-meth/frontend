...
const PRIVATE_IP_PATTERNS = [/^http:\/\/localhost/i, /^http:\/\/127\.0\.0\.1/i, /^http:\/\/192\.168\./i, /^http:\/\/10\./i, /^http:\/\/172\.(1[6-9]|2\d|3[01])\./i];
const stateChangingMethods = ["POST", "PUT", "PATCH", "DELETE"];
const excludeCsrfRoutes = ["csrf-token", "payments/webhook"];
const idempotentMethods = ["POST", "PUT", "PATCH", "DELETE"];
const excludeIdempotencyRoutes = ["auth/login", "auth/register", "auth/refresh", "auth/refresh-token", "payments/create-order"];
const excludedRoutes = ["/auth/login", "/auth/register", "/auth/logout", "/auth/refresh"];
...
export const getBaseUrl = () => {
  return import.meta.env.VITE_API_BASE_URL || "https://api.medassist.viyaninfo.com/api";
};
...
export async function getCsrfToken(forceRefresh = false) {
  ...
  csrfPromise = axios.get(`${getBaseUrl()}/csrf-token`, {
    withCredentials: true,
    headers: {
      ...(import.meta.env.DEV && {
        "ngrok-skip-browser-warning": "69420"
...
api.interceptors.response.use(response => {
  ...
  const isExcluded = excludedRoutes.some(route => originalRequest?.url?.includes(route));
  ...
  if (status >= 500) {
    const errorMsg = error.response?.data?.error?.message || error.response?.data?.message || "Internal Server Error";
    console.error(`[API] Server error (${status}):`, errorMsg);
    if (status === 503) {
      window.dispatchEvent(new CustomEvent("api:serviceUnavailable", {
        detail: {
  ...
  if (error.response?.data?.error?.message) {
    ...
    error.message = typeof error.response.data.error === "string" ? error.response.data.error : error.response.data.error.message || error.message;
  }
  return Promise.reject(error);
});
export default api;