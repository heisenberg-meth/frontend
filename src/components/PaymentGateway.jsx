  ...
  useEffect(() => {
    return () => {
      if (razorpayRef.current) {
        try { razorpayRef.current.close(); } catch (e) { void e; }
    ...
    return (
      <div className="dark bg-[var(--bg-dark)] min-h-screen flex items-center justify-center p-6 font-['Manrope']">
        <div className="bg-[var(--surface)] border border-[var(--surface)] rounded-2xl p-12 max-w-md w-full text-center shadow-2xl animate-in">
          ...
          <h2 className="text-3xl font-bold text-on-surface mb-4">Licensing Authorized</h2>
           <p className="text-on-surface-variant mb-10">Your facility has been upgraded successfully.</p>
          <button onClick={onPaymentComplete} className="w-full bg-primary text-[var(--bg-dark)] py-4 rounded-xl font-extrabold shadow-lg hover:brightness-110 transition-all flex items-center justify-center gap-3">
        ...
        <div className="flex flex-col md:flex-row gap-8">
          <div className="w-full md:w-5/12 lg:w-4/12">
            <div className="bg-[var(--surface)] rounded-xl border border-[var(--surface)] p-6 sticky top-24 shadow-2xl">
              <h3 className="text-2xl font-bold mb-6 text-on-surface">Order Summary</h3>
              <div className="flex items-center justify-between py-4 border-b border-[var(--surface)]">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-on-surface-variant">Annual Subscription</p>
                  </div>
                </div>
                <p className="font-bold text-primary">₹{amount}</p>
              </div>
              <div className="mt-6 space-y-3">
                <div className="flex justify-between text-sm text-on-surface-variant"><span>Subtotal</span><span>₹{amount}.00</span></div>
                <div className="pt-4 border-t border-[var(--surface)] flex justify-between items-center"><span className="text-lg font-bold text-on-surface">Total</span><span className="text-3xl font-extrabold text-primary">₹{amount}</span></div>
              </div>
            </div>
          </div>
          ...
          <div className="w-full md:w-7/12 lg:w-8/12">
            <div className="bg-[var(--surface)] rounded-xl border border-[var(--surface)] p-8 shadow-2xl">
              <h2 className="text-3xl font-bold mb-8 text-on-surface">Complete Payment</h2>
              <div className="grid grid-cols-3 gap-4 mb-8">
                <button onClick={() => setMethod("card")} className={`flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all ${method === "card" ? "border-primary bg-primary/10 text-primary" : "border-[var(--surface)] bg-[var(--bg-dark)] text-on-surface-variant"}`}>
                ...
                <button onClick={() => setMethod("upi")} className={`flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all ${method === "upi" ? "border-primary bg-primary/10 text-primary" : "border-[var(--surface)] bg-[var(--bg-dark)] text-on-surface-variant"}`}>
                ...
                <button onClick={() => setMethod("qr")} className={`flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all ${method === "qr" ? "border-primary bg-primary/10 text-primary" : "border-[var(--surface)] bg-[var(--bg-dark)] text-on-surface-variant"}`}>
                  <span className="material-symbols-outlined text-3xl mb-1">qr_code_2</span>
                  <span className="text-xs font-bold uppercase tracking-wider">QR Code</span>
                </button>
              </div>

              <div className="p-6 bg-[var(--surface)] rounded-xl border border-primary/20 text-center">
                <p className="text-on-surface-variant text-sm mb-6 italic">Razorpay secure checkout will launch to handle your {method.toUpperCase()} payment safely.</p>
                {paymentError && <div style={{ background: "rgba(239,68,68,0.1)", border: "1px solid #EF4444", borderRadius: "8px", padding: "12px 16px", marginBottom: "12px", color: "#EF4444", fontSize: "13px" }}>{paymentError}</div>}
                <button onClick={handlePay} disabled={status === "processing"} className="w-full bg-primary hover:brightness-110 text-[var(--bg-dark)] font-extrabold py-5 rounded-xl shadow-lg flex items-center justify-center gap-3 active:scale-[0.98] transition-all disabled:opacity-50">
