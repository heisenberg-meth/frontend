...
import { Loader2, AlertTriangle, Search, ChevronRight, ArrowLeft, TrendingUp, Trash2, X, Activity, Download, FileText } from "lucide-react";
import { AnimatePresence, m } from "framer-motion";
import jsPDF from "jspdf";
import autoTable from "jspdf-autotable";
import api from "../../api.js";
import { API_ROUTES } from "../../constants/api.routes.js";
const headers = ["Medicine", "Batch", "MFG Date", "Expiry Date", "Days Left", "Qty", "Value (₹)", "Supplier"];
function ExpiryReportSection1({
  e,
  setExpiryFilter
}) {
  return <div className="reports-kpi-grid" style={{
    marginTop: "20px"
  }}>
        <div role="button" tabIndex={0} className="report-kpi-card" onKeyDown={e => {
      if (e.key === "Enter" || e.key === " ") {
        e.preventDefault();
        e.currentTarget.click();
      }
    }} onClick={() => setExpiryFilter("Expired")} style={{
      cursor: "pointer"
    }}>
    ...
    }} onClick={() => setExpiryFilter("< 7 Days")} style={{
      cursor: "pointer"
    }}>
          <div className="stat-label" style={{
        color: "var(--danger)"
    ...
    }} onClick={() => setExpiryFilter("7-30 Days")} style={{
      cursor: "pointer"
    }}>
          <div className="stat-label" style={{
        color: "var(--warning)"
    ...
    }} onClick={() => setExpiryFilter("30-90 Days")} style={{
      cursor: "pointer"
    }}>
          <div className="stat-label" style={{
        color: "var(--info)"
        ...
        <table className="purchase-table">
          ...
          <tbody>
            ...
            color: "var(--text-muted)"
              ...
              </tr> : filteredStock.map(item => <tr key={item.id} className={item.urgency === "danger" ? "expiry-row-danger" : item.urgency === "warning" ? "expiry-row-warning" : item.urgency === "info" ? "expiry-row-info" : ""}>
                  <td>
                    <div style={{
              fontWeight: 700
            }}>{item.medicineName}</div>
                  </td>
                  ...
                  <td>
                    <b style={{
              color: item.daysLeft < 10 ? "var(--danger)" : item.daysLeft < 30 ? "var(--warning)" : "var(--text)"
            }}>
            ...
            fontWeight: 700
              ...
              alignItems: "center"
                ...
                color: "var(--warning)"
              }} title="Return" onClick={() => handleAction("Return", item)}>
                        <ArrowLeft size={12} />
                      </button>
                      <button className="micro-btn" style={{
                color: "var(--info)"
              }} title="Discount" onClick={() => handleAction("Discount", item)}>
                        <TrendingUp size={12} />
                      </button>
                      <button className="micro-btn" style={{
                color: "var(--danger)"
              }} title="Dispose" onClick={() => promptDeleteBatch(item)}>
                        <Trash2 size={12} />
                      </button>
                    </div>
                  </td>
...
function ExpiryReportSection3({
  ...
  return <AnimatePresence>
        {showActionModal && <div className="stock-modal-overlay" style={{
      zIndex: 1100
    }}>
        ...
        scale: 0.9
          ...
          textAlign: "left"
                ...
                <div className="selected-batch-info" style={{
            ...
            marginBottom: "20px"
              ...
              fontSize: "12px"
                ...
                {actionType === "Return" && <div className="pos-input-group">
                ...
                {actionType === "Discount" && <div className="pos-input-group">
                    <label htmlFor="field_00six4" className="p-label">
                      DISCOUNT PERCENTAGE (%)
                    </label>
                    <input id="field_00six4" required className="pos-input" type="number" placeholder="20" defaultValue={15} />
                  </div>}
            ...
            marginTop: "16px"
                  ...
                  <textarea id="field_y4jgfc" className="pos-input" style={{
              minHeight: "80px"
            }} placeholder={`Details for ${actionType.toLowerCase()} process...`} value={actionNotes} onChange={e => setActionNotes(e.target.value)} />
                </div>
              </div>
        ...
        {showDeleteConfirm && <div className="stock-modal-overlay" style={{
      zIndex: 1100
    }}>
            <m.div className="stock-modal-content" style={{
        width: "380px"
        ...
        scale: 0.9
          ...
          textAlign: "center"
            ...
            flex: 1
          }} onClick={confirmDeleteBatch} disabled={isDeleting}>
...
export default function ExpiryReport({
  ...
  const fetchExpiry = async () => {
    ...
    try {
      ...
      if (res.data && res.data.success) {
        ...
        const stockData = Array.isArray(apiData) ? apiData : apiData?.report || [];
  ...
  const processStockItem = item => {
    ...
    let urgency = "safe";
    if (days < 0) urgency = "danger";else if (days < 30) urgency = "warning";else if (days < 90) urgency = "info";
    return {
      ...item,
      daysLeft: days,
      urgency,
  ...
  const processedStock = (Array.isArray(expiryStock) ? expiryStock : []).map(processStockItem);
  const expiredCount = processedStock.filter(i => i.daysLeft < 0).length;
  const expiring7DaysCount = processedStock.filter(i => i.daysLeft >= 0 && i.daysLeft < 7).length;
  const expiring7To30DaysCount = processedStock.filter(i => i.daysLeft >= 7 && i.daysLeft < 30).length;
  ...
  const exportCSV = () => {
    ...
    const rows = filteredStock.map(item => [item.medicineName, item.batchNumber, item.manufacturingDate ? new Date(item.manufacturingDate).toLocaleDateString() : "N/A", new Date(item.expiryDate).toLocaleDateString(), item.daysLeft <= 0 ? "EXPIRED" : `${item.daysLeft} Days`, item.quantity, item.value, item.supplierName || "N/A"]);
  ...
  const exportPDF = () => {
    ...
    try {
      ...
      autoTable(doc, {
        ...
        body: filteredStock.map(item => [item.medicineName, item.batchNumber, new Date(item.expiryDate).toLocaleDateString(), item.daysLeft <= 0 ? "EXPIRED" : `${item.daysLeft} Days`, item.quantity, `₹${item.value}`, item.supplierName || "N/A"]),
        styles: {
          fontSize: 10
        },
        headStyles: {
  ...
  if (loading) {
    return <div className="reports-loading" style={{
      display: "flex",
  ...
  if (errorState) {
    return <div className="empty-state-card" style={{
      padding: "40px",
      textAlign: "center",
      ...
      borderRadius: "8px"
        ...
        <AlertTriangle size={36} color="var(--danger)" style={{
        marginBottom: "12px"
      }} />
        <h4 style={{
        fontWeight: 700,
  ...
  return <>
      ...
      gap: "8px"
        ...
        {["All", "Expired", "< 7 Days", "7-30 Days", "30-90 Days"].map(p => <button key={p} className={`p-tab ${expiryFilter === p ? "active" : ""}`} style={{
      ...
      <ExpiryReportSection1 e={e} setExpiryFilter={setExpiryFilter} />

      <div role="button" tabIndex={0} className="expiry-urgency-banner" onKeyDown={e => {
      if (e.key === "Enter" || e.key === " ") {
        e.preventDefault();
        e.currentTarget.click();
      }
      ...
      marginTop: "20px"
        ...
        flex: 1
          ...
          <div style={{
          fontWeight: 700
        }}>Critical Attention Required</div>
      ...
      flexWrap: "wrap"
        ...
        flex: 1
          ...
          <Search size={16} />
          <><label htmlFor="field_29gzgr" className="sr-only">Search medicine or batch...</label><input required type="text" placeholder="Search medicine or batch..." style={{
            background: "none",
            border: "none",
            color: "var(--text-main)",
            ...
            width: "100%",
            outline: "none"
          }} value={expirySearch} onChange={e => setExpirySearch(e.target.value)} id="field_29gzgr" /></>
        </div>

        {filteredStock.length > 0 && <div style={{
        display: "flex",
        gap: "8px"
      }}>
            <button className="pos-btn outline" onClick={exportCSV} style={{
          padding: "8px 12px",
          fontSize: "12px"
        }}>
              <Download size={14} /> CSV
            </button>
            <button className="pos-btn outline" onClick={exportPDF} style={{
          padding: "8px 12px",
          fontSize: "12px"
        }}>
      ...
      <ExpiryReportSection2 handleAction={handleAction} item={item} promptDeleteBatch={promptDeleteBatch} />

      {/* Action Dialog Overlay */}
      <ExpiryReportSection3 setShowActionModal={setShowActionModal} setActionNotes={setActionNotes} actionLoadingRef={actionLoadingRef} setActionLoading={setActionLoading} actionType={actionType} selectedItem={selectedItem} actionNotes={actionNotes} showToast={showToast} fetchExpiry={fetchExpiry} setShowDeleteConfirm={setShowDeleteConfirm} />
    </>;
}