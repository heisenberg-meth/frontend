...
function PurchaseReportSection1({
  comparisonData,
  e,
  navigate
}) {
  return <div className="report-charts-grid">
        ...
        gap: "20px"
            ...
            {comparisonData.length === 0 ? <div style={{
          flex: 1,
          textAlign: "center",
          color: "var(--text-muted)"
        }}>
                No trend comparison data available.
              </div> : comparisonData.map(val => {
        ...
        marginTop: "32px"
            ...
            {supplierSpend.map(s => <div role="button" tabIndex={0} key={s.name} className="med-revenue-row" onKeyDown={e => {
...
export default function PurchaseReport({
  ...
  useEffect(() => {
    ...
    const fetchPurchases = async () => {
      ...
      setErrorState(null);
      try {
        const res = await api.get(API_ROUTES.REPORTS_PURCHASES || "reports/purchases", {
          params: {
            from,
            to
          }
        ...
        if (!ignore) {
          setErrorState(err.response?.data?.error || err.message || "Failed to load purchase report");
        }
      } finally {
  ...
  return <>
      ...
      <PurchaseReportSection1 comparisonData={comparisonData} e={e} navigate={navigate} />
    </>;
}