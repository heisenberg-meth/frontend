...
import { TrendingUp, Receipt, Activity, ArrowUpRight, ArrowDownRight, Download, FileText, Printer, Loader2, AlertTriangle } from "lucide-react";
...
function SalesReportSection1({
  ...
  return <div className="report-charts-grid">
        <div className="pos-card">
          <div className="pos-card-title">Top medicines by Revenue</div>
          <div style={{
        marginTop: "20px"
      }}>
            {topMedicines.map(m => <div role="button" tabIndex={0} key={m.revenue} className="med-revenue-row" onKeyDown={e => {
          if (e.key === "Enter" || e.key === " ") {
            ...
            e.currentTarget.click();
                ...
                <div className="med-bar-bg">
                  <div className="med-bar-fill" style={{
            ...
            {topMedicines.length === 0 && <div className="result-meta" style={{
          textAlign: "center",
          padding: "20px"
        }}>
        ...
        marginTop: "20px"
          ...
          background: donutBg
            ...
            <div className="donut-legend">
              {paymentMethods.map((pm, idx) => <div className="legend-item" key={pm.method}>
                  <div className="legend-dot" style={{
              background: idx === 0 ? "var(--primary)" : idx === 1 ? "var(--info)" : "var(--success)"
            }} />
...
function SalesReportSection2() {
  return <div id="sales-report-table">
        <div className="purchase-table-card">
          <table className="purchase-table">
            ...
            <tbody>
              {data.chart.map(d => <tr key={d.date}>
                  <td>{d.date}</td>
                  <td style={{
              fontWeight: 700,
              textAlign: "right"
...
export default function SalesReport({
  ...
  const exportCSV = () => {
    ...
    const rows = data.chart.map(item => [item.date, item.bills, item.revenue]);
  ...
  const exportPDF = () => {
    ...
    try {
      ...
      autoTable(doc, {
        ...
        body: data.chart.map(item => [item.date, item.bills, `${item.revenue.toLocaleString()}`]),
        styles: {
          fontSize: 10
        },
        headStyles: {
          fillColor: [79, 219, 200]
        }
      });
  ...
  const renderLineChart = () => {
    ...
    const points = data.chart.map((d, i) => {
      const x = i / (data.chart.length - 1 || 1) * width;
      const y = height - d.revenue / maxRevenue * height;
      return `${x},${y}`;
    }).join(" ");
    const areaPoints = `${points} ${width},${height} 0,${height}`;
    return <svg viewBox={`0 0 ${width} ${height}`} className="line-chart-svg" style={{
      width: "100%",
      height: "100%"
    }}>
        <defs>
  ...
  if (!data || !data.chart.length) {
    return <div className="empty-state-card" style={{
      ...
      textAlign: "center"
        ...
        <Receipt size={40} color="var(--text-muted)" style={{
        marginBottom: "12px"
      }} />
        <h4 style={{
        fontWeight: 700
      }}>No sales data found</h4>
        <p style={{
        color: "var(--text-muted)",
        marginTop: "6px"
      }}>
  ...
  const paymentMethods = [{
    method: "Cash",
    percentage: summary.grossRevenue > 0 ? Math.round(paymentDistribution.cash / summary.grossRevenue * 100) : 0
  }, {
    method: "UPI",
    percentage: summary.grossRevenue > 0 ? Math.round(paymentDistribution.upi / summary.grossRevenue * 100) : 0
  }, {
    method: "Card",
    percentage: summary.grossRevenue > 0 ? Math.round(paymentDistribution.card / summary.grossRevenue * 100) : 0
  }];
  const totalPercent = paymentMethods.reduce((sum, method) => sum + method.percentage, 0);
  if (totalPercent > 100.1) {
    console.error(`[Validation Error] Payment distribution exceeds 100%: ${totalPercent}%`);
  }
  ...
  const donutBg = (() => {
    ...
    if (pCash > 0) activeSegments.push({
      color: "var(--primary)",
      value: pCash
    });
    if (pUpi > 0) activeSegments.push({
      color: "var(--info)",
      value: pUpi
    });
    if (pCard > 0) activeSegments.push({
      color: "var(--success)",
      value: pCard
    });
    ...
    const parts = activeSegments.map((seg, idx) => {
      ...
      const end = idx === activeSegments.length - 1 ? 100 : currentPos + seg.value;
      currentPos = end;
  ...
  return <>
      <div className="reports-kpi-grid">
        ...
        icon: Activity,
        col: "var(--info)"
      }].map(kpi => <div key={kpi.label} className="report-kpi-card">
            <div className="stat-card-header">
              <span className="stat-label">{kpi.label}</span>
              <div className="stat-icon" style={{
            backgroundColor: `${kpi.col}15`,
            color: kpi.col
          }}>
                <kpi.icon size={16} />
              </div>
            ...
            <div className={`kpi-trend ${kpi.dir}`}>
              {kpi.dir === "up" ? <ArrowUpRight size={14} /> : <ArrowDownRight size={14} />}
              {kpi.trend}
            </div>
          </div>)}
      </div>
      ...
      <SalesReportSection1 e={e} showToast={showToast} m={m} topMedicines={topMedicines} />
