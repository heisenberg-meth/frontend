...
function MedicineRow({ item, idx, onUpdate, onRemove, showToast, theme = "light" }) {
  ...
  const selectMedicine = (med) => {
    ac.clear();
    const price = Number(med.price ?? med.mrp ?? med.salePrice ?? med.unitPrice ?? 0);
    const gstVal = Number(med.gst ?? med.gstPercentage ?? med.gstRate ?? med.taxRate ?? 0);
    const stockVal = Number(med.stock ?? med.quantity ?? med.availableStock ?? 0);
  ...
  const handleBatchChange = (e) => {
    ...
    if (found) {
      onUpdate(idx, "_batchSelect", {
        ...
        ...(found.mrp ? { price: Number(found.mrp), mrp: Number(found.mrp) } : {}),
      });
    }
  };
  ...
  const handleKeyDown = (e) => {
    if (e.key === "Escape") { ac.setOpen(false); return; }
    ...
    <div className="med-row" ref={containerRef}>
      ...
      <div className="med-cell med-cell-name">
        <div className="med-search-wrap">
          ...
          {ac.open && (ac.results.length > 0 || (!ac.loading && ac.query.length >= 2)) &&
            createPortal(
              <div
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="ac-item-right">
                          <div className="ac-item-price">₹{medPrice.toFixed(2)}</div>
                          <div className={`ac-item-stock ${
                            medStock === 0 ? "ac-item-stock-zero" : medStock < 10 ? "ac-item-stock-low" : ""
                          }`}>
        ...
        {item.batchesLoading ? (
          ...
          >
            ...
            {isSelected && (item.batches || []).length === 0 && (
              <option value={item.batchId || ""}>{item.batchNumber || "Default batch"}</option>
            )}
            {(item.batches || []).map((b) => {
              const bid = b.id || b._id || b.batchId;
              const bnum = b.batchNumber || b.batch_number || bid;
              const exp = b.expiryDate ? new Date(b.expiryDate).toLocaleDateString("en-IN", { month: "short", year: "2-digit" }) : "";
              const qty = b.quantity ?? b.stock ?? b.availableQty ?? 0;
              return (
                <option key={bid} value={bid}>
                  {bnum}{exp ? ` | Exp:${exp}` : ""} | Qty:{qty}
                </option>
              );
            })}
          </select>
      ...
      <div className="med-cell med-cell-amount">
        <span className="med-amount-text">
          ₹{rowAmount.toFixed(2)}
        </span>
      </div>
...
// ─── Main section ─────────────────────────────────────────────────────────────
export default function MedicineTableSection({ lineItems, setLineItems, showToast, theme = "light" }) {
