...
import { AlertTriangle, Clock, Package, IndianRupee, Receipt, Zap, Calendar, BadgeX, CheckCircle, Trash2 } from "lucide-react";
...
const fmt = n => `₹${safeNumber(n).toLocaleString("en-IN", {
...
function DashboardSection1({
  ...
  return <div className="hub-stats-row-v2">
        <div role="button" tabIndex={0} className="stat-card-v2" onKeyDown={e => {
      if (e.key === "Enter" || e.key === " ") {
        e.preventDefault();
        e.currentTarget.click();
      }
    }} onClick={() => navigate("/stock")} onMouseMove={handleMouseMove}>
    ...
    }} onClick={() => navigate("/lowstock")} onMouseMove={handleMouseMove}>
          <div className="stat-v2-header">
    ...
    }} onClick={() => navigate("/expiry")} onMouseMove={handleMouseMove}>
          <div className="stat-v2-header">
    ...
    }} onClick={() => navigate("/billing")} onMouseMove={handleMouseMove}>
          <div className="stat-v2-header">
    ...
    }} onClick={() => navigate("/purchases")} onMouseMove={handleMouseMove}>
          <div className="stat-v2-header">
...
function DashboardSection2({
  ...
  return <div className="hub-main-grid">
        {/* LEFT COLUMN 40% */}
          ...
          <div className="bento-card" onMouseMove={handleMouseMove}>
            <div className="flex justify-between items-center mb-6">
              <div className="flex items-center gap-3">
                ...
                {stats.lowStock > 0 && <span className="count-badge warning">{stats.lowStock}</span>}
              </div>
              <button className="view-all-link text-primary font-bold text-[13px] hover:underline cursor-pointer" onClick={() => navigate("/lowstock")}>
                View All →
              </button>
            </div>
            ...
            <div className="mini-table-v2">
              ...
              <div className="mini-table-body flex flex-col gap-1">
                {needsReorderList.length > 0 ? needsReorderList.map(item => <div role="button" tabIndex={0} key={item.id} className="mini-table-row" onKeyDown={e => {
              if (e.key === "Enter" || e.key === " ") {
                e.preventDefault();
                e.currentTarget.click();
              }
                      ...
                      <span className="row-act">
                        <button className="micro-btn-teal" onClick={e => {
                  e.stopPropagation();
                  navigate("/purchases", {
                    state: {
                      action: "raise-po",
                    ...
                    </div>) : <div className="text-center py-6 text-on-surface-variant text-sm">
          ...
          <div className="urgent-expiries-card" onMouseMove={handleMouseMove}>
            <div className="urgent-expiries-header">
              ...
              <div className="urgent-header-text">
                ...
                <p>
                  {dashboardData?.expiryMetrics ? dashboardData.expiryMetrics.expiredProducts ?? dashboardData.expiryMetrics.expired : dashboardData?.expiredOverview?.totalExpiredProducts ?? expiring.length}{" "}
                  products require disposal
                </p>
              </div>
            </div>

            {dashboardData?.expiredOverview ? <div style={{
          padding: "0 20px 16px"
        }}>
                <div className="disposal-summary-card compact-summary">
                  <div>
                    ...
                    <b>
                      {dashboardData?.expiryMetrics ? dashboardData.expiryMetrics.expiredProducts ?? dashboardData.expiryMetrics.expired : dashboardData.expiredOverview.totalExpiredProducts}
                    </b>
                  </div>
                  <div>
                    <span>Total Units:</span>{" "}
                    <b>
                      {dashboardData?.expiryMetrics?.expiredUnits != null ? dashboardData.expiryMetrics.expiredUnits : dashboardData.expiredOverview.totalUnits}
                    </b>
                  </div>
                  <div>
                    ...
                    <b className="text-rose-500">
                      ₹
                      {safeNumber(dashboardData?.expiryMetrics?.expiredValue != null ? dashboardData.expiryMetrics.expiredValue : dashboardData.expiredOverview.totalInventoryValue).toLocaleString("en-IN")}
                    </b>
                  </div>
                </div>

                <button className="urgent-disposal-btn" style={{
            marginTop: 12
          }} onClick={e => {
            e.stopPropagation();
              ...
              marginRight: 7
                ...
                <div className="urgent-table">
                  ...
                  <div className="urgent-table-body">
                    {expiring.length === 0 ? <div className="flex flex-col items-center justify-center py-10 gap-3 text-on-surface-variant">
                ...
                return <div key={m.id} className="urgent-table-row">
                            <span className="urgent-med-name">{m.name}</span>
                            <span className="urgent-batch">
                              {m.batchNumber || "B-001"}
                            </span>
                            <span className="urgent-days">
                              <div className={`urgent-badge ${isOverdue ? "overdue" : "warning"}`}>
                                {isOverdue ? ` ${displayDays} Days Overdue` : ` ${displayDays} Days Left`}
                              </div>
                            </span>
                ...
                {(dashboardData?.expiryMetrics ? dashboardData.expiryMetrics.expiredProducts ?? dashboardData.expiryMetrics.expired : dashboardData?.expiredOverview?.totalExpiredProducts ?? expiring.length) > 0 && <button className="urgent-disposal-btn" onClick={e => {
            e.stopPropagation();
            navigate("/expiry/bulk-disposal");
          }}>
...
export default function Dashboard({
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      try {
        setIsLoading(true);
        const [overviewRes, salesRes, expiredRes, metricsRes, reconRes] = await Promise.all([api.get("/dashboard/overview"), api.get("/dashboard/sales-summary"), api.get("/inventory/expired/overview").catch(() => null), api.get("/inventory/expiry-metrics").catch(() => null), api.get("/inventory/reconciliation").catch(() => null)]);
  ...
  const stats = useMemo(() => {
    ...
    if (backendInv) {
      return {
        ...
        expiring: expiryMetrics ? expiryMetrics.expiring30Products ?? expiryMetrics.expiring30 ?? 0 : backendInv.expiring30d ?? 0,
        lowStock: backendInv.lowStock ?? 0,
        inventoryValue: backendInv.inventoryValue ?? 0
      };
    }
    const total = medicines?.length ?? 0;
    const expiringCount = (medicines || []).filter(m => getStock(m) > 0 && getDays(getExpiry(m)) <= (expiryDays || 30)).length;
    const lowCount = (medicines || []).filter(m => getStock(m) <= (m.reorderLevel ?? m.reorderPoint ?? lowStock ?? 10)).length;
    const totalValue = (medicines || []).reduce((s, m) => s + safeNumber(getStock(m)) * safeNumber(getPrice(m)), 0);
    return {
      total,
  ...
  const expiring = (medicines || []).filter(m => getStock(m) > 0 && getDays(getExpiry(m)) <= (expiryDays || 30)).sort((a, b) => getDays(getExpiry(a)) - getDays(getExpiry(b)));
  const needsReorderList = (medicines || []).filter(m => getStock(m) > 0 && getStock(m) <= (m.reorderLevel ?? m.reorderPoint ?? lowStock ?? 10)).slice(0, 5);
  ...
  if (!isLoading && !dashboardData) {
    return <div className="hub-container" style={{
      display: "flex",
      flexDirection: "column",
      alignItems: "center",
      justifyContent: "center",
  ...
  return <div className="hub-container">
      ...
      {showBanner && user?.subscriptionStatus === "TRIAL" && <div className="announcement-banner">
          ...
          <div className="banner-right">
            ...
            <button className="banner-dismiss" onClick={() => setShowBanner(false)}>
              <BadgeX size={24} />
            </button>
          </div>
        </div>}
      ...
      <DashboardSection1 e={e} navigate={navigate} />

      {/* 5. MAIN GRID */}
      <DashboardSection2 navigate={navigate} e={e} item={item} />
