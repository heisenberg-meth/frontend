...
import { ArrowLeft, Activity, Layers, Clock, FileText, Loader2 } from "lucide-react";
import { lazy, Suspense } from "react";
const LazyRechartsWrapper = lazy(() => import("recharts").then(m => ({
  default: function Chart({
    categories,
    COLORS
  }) {
    return <m.ResponsiveContainer width="100%" height="100%">
          <m.PieChart>
            <m.Pie data={categories} cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={5} dataKey="value" nameKey="category">
              {categories.map((entry, index) => <m.Cell key={`cell-${entry.category}`} fill={COLORS[index % COLORS.length]} />)}
            </m.Pie>
            <m.Tooltip formatter={value => formatIndianCurrency(value)} contentStyle={{
    ...
    gap: "2rem"
        ...
        gap: "1.5rem"
            ...
            <div className="inventory-summary-card">
              ...
              <div className="inventory-summary-value" style={{
            color: "#4fdbc8"
          }}>
          ...
          <section className="inventory-card-container">
            ...
            <div className="inventory-table-scroll">
              <table className="inventory-analytics-table">
                ...
                <tbody>
                  {highValueStock.map(item => <tr key={item.name}>
                      <td>
                        {item.name}{" "}
                        <span style={{
                    fontSize: "0.75rem",
          ...
          <section className="inventory-card-container">
            ...
            <div className="inventory-table-scroll">
              <table className="inventory-analytics-table">
                ...
                <tbody>
                  {riskItems.map(item => <tr key={item.name}>
                      <td>{item.name}</td>
                      <td className="numeric">
          ...
          <section className="inventory-card-container">
            ...
            <div className="inventory-table-scroll">
              <table className="inventory-analytics-table">
                ...
                <tbody>
                  {deadStock.map(item => <tr key={item.name}>
                      <td>{item.name}</td>
                      <td className="numeric">
                        {formatIndianCurrency(item.value)}
                      </td>
    ...
    const fetchAnalytics = async () => {
      try {
        ...
        setAnalytics({
          ...
          categories: Object.keys(categoryMap).map(k => ({
            category: k,
