...
import { AlertTriangle, Trash2, Download, Search, X, CheckCircle2, IndianRupee, Calendar, Package, Layers, Clock } from "lucide-react";
...
function BulkDisposalSection1() {
  return overview && <div style={{
    display: "grid",
    ...
    marginBottom: 24
      ...
      cursor: "default"
            ...
            <div className="stat-v2-val danger text-rose-500">
              {expiryMetrics ? expiryMetrics.expiredProducts ?? expiryMetrics.expired : overview.totalExpiredProducts}
            </div>
          </div>
          <div className="stat-card-v2" style={{
      cursor: "default"
      ...
      textAlign: "left"
        ...
        marginBottom: 16
          ...
          fontWeight: 600
            ...
            {result.skippedCount > 0 && result.items && <div style={{
        marginTop: 16,
        paddingTop: 16,
        borderTop: "1px solid var(--outline-variant)"
          ...
          color: "var(--text-dim)"
            ...
            if (item.status === "SKIPPED") {
              acc.push(<li key={item.reason} style={{
                marginBottom: 4
              }}>
      ...
      flexWrap: "wrap"
        ...
        flex: 1
          ...
          maxWidth: 320
                ...
                <><label htmlFor="field_b6lhi9" className="sr-only">Search medicine or batch...</label><input type="text" placeholder="Search medicine or batch..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="search-input" id="field_b6lhi9" /></>
        ...
        fontSize: 13
            ...
            background: "var(--surface-container)"
              ...
              width: 40
                    ...
                    <input type="checkbox" checked={selectAll} onChange={toggleSelectAll} style={{
              ...
              color: "var(--text-muted)"
                      ...
                      {batches.length === 0 ? "No expired medicines found" : "No results match your search"}
                    </td>
                  </tr> : filtered.map(b => <tr key={b.batchId} style={{
            borderBottom: "1px solid var(--outline-variant)",
            background: selected.has(b.batchId) ? "var(--overlay-05)" : "transparent",
            cursor: "pointer",
            transition: "background 0.15s"
          }} onMouseEnter={e => {
            if (!selected.has(b.batchId)) e.currentTarget.style.background = "var(--overlay-03)";
          }} onMouseLeave={e => {
            if (!selected.has(b.batchId)) e.currentTarget.style.background = "transparent";
          }} onKeyDown={e => {
            if (e.key === "Enter" || e.key === " ") {
              ...
              padding: "10px 16px"
                        ...
                        <input type="checkbox" checked={selected.has(b.batchId)} onChange={() => toggleSelect(b.batchId)} style={{
                accentColor: "var(--primary)",
                cursor: "pointer"
              }} />
                      </td>
...
function BulkDisposalSection3({
  setReason,
  setShowConfirmModal
}) {
  return <AnimatePresence>
        {showConfirmModal && <div className="modal-overlay">
            <m.div className="urgent-disposal-modal" style={{
        maxWidth: 520
      }} initial={{
        opacity: 0,
        scale: 0.95,
        ...
        y: 20
          ...
          marginBottom: 4
              ...
              <div className="disposal-summary-card compact-summary" style={{
          margin: "16px 0"
        }}>
                <div>
          ...
          marginBottom: 16
                ...
                <label htmlFor="field_ygyctc" style={{
            fontSize: 13,
            fontWeight: 600,
            display: "block",
            marginBottom: 6
                ...
                <select id="field_ygyctc" value={reason} onChange={e => setReason(e.target.value)} style={{
            width: "100%",
            padding: "10px 12px",
            borderRadius: 8,
            border: "1px solid var(--outline-variant)",
            ...
            fontSize: 13
              ...
              <div className="disposal-risk-card compact-risk" style={{
          marginBottom: 16
        }}>
              ...
              <div className="urgent-modal-actions sticky-actions">
                <button className="urgent-modal-btn-cancel" onClick={() => setShowConfirmModal(false)} disabled={disposing}>
                  Cancel
                </button>
                <button className="urgent-modal-btn-verify flex items-center justify-center gap-2" disabled={disposing} onClick={handleDispose} style={{
...
export default function BulkDisposal({
  showToast
}) {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const [batchRes, overviewRes, metricsRes] = await Promise.all([api.get("/inventory/expired"), api.get("/inventory/expired/overview"), api.get("/inventory/expiry-metrics").catch(() => null)]);
  ...
  const filtered = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      result = result.filter(b => b.medicineName?.toLowerCase().includes(q) || b.batchNumber?.toLowerCase().includes(q) || b.genericName?.toLowerCase().includes(q));
    }
    return result;
  }, [batches, searchQuery]);
  ...
  const handleDispose = async () => {
    ...
    try {
      ...
      const disposedItems = Array.isArray(data) ? data.filter(d => d.status === "DISPOSED") : [];
      const skippedItems = Array.isArray(data) ? data.filter(d => d.status === "SKIPPED") : [];
      setResult({
      ...
      showToast?.("Disposal failed: " + (err.response?.data?.error?.message || err.message), "error");
    } finally {
  ...
  return <div className="hub-container">
      <div className="hub-header">
        ...
        <div className="hub-status-group" style={{
        display: "flex",
        alignItems: "center",
        gap: 10
      }}>
      ...
      <BulkDisposalSection2 acc={acc} item={item} setSearchQuery={setSearchQuery} setShowConfirmModal={setShowConfirmModal} selectedData={selectedData} selected={selected} b={b} e={e} toggleSelect={toggleSelect} />

      <BulkDisposalSection3 setReason={setReason} setShowConfirmModal={setShowConfirmModal} />
    </div>;
}
function exportCSV(data) {
  if (!data || data.length === 0) return;
  const rows = data.map(b => [b.medicineName, b.batchNumber, format(new Date(b.expiryDate), "MMM-yyyy"), b.quantity, b.totalValue]);
