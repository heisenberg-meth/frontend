...
import { Plus, MessageSquare, CheckCircle2, AlertTriangle, Send } from "lucide-react";
import { AnimatePresence, m } from "framer-motion";
import api from "../api";
const CATEGORIES = ["INVENTORY", "BILLING", "PURCHASE", "SUPPLIER", "SALES", "REPORTS", "IMPORT", "ACCOUNT", "TECHNICAL", "OTHER"];
...
function SupportTicketsSection1({
  e,
  loadTicketDetails,
  ticket
}) {
  return loading ? <div style={{
    textAlign: "center",
    padding: 40
  }}>Loading...</div> : tickets.length === 0 ? <div style={{
    textAlign: "center",
    padding: 40,
    color: "var(--text-dim)"
  }}>
    ...
    gap: 8
          ...
          {tickets.map(ticket => <m.div role="button" tabIndex={0} key={ticket.id} initial={{
      opacity: 0,
      y: 10
    }} animate={{
      opacity: 1,
      y: 0
    }} className="sub-current-card" style={{
      cursor: "pointer",
      padding: 16
    }} onKeyDown={e => {
...
function SupportTicketsSection2({
  ...
  return <AnimatePresence>
        {showCreate && <div className="modal-overlay-v2">
        ...
        scale: 0.95
              ...
              <div className="modal-body-v2">
                <div className="input-v2">
                  ...
                  <input id="field_4zhc6f" type="text" value={form.title} onChange={e => setForm({
              ...form,
              title: e.target.value
                ...
                <div className="input-v2">
                  ...
                  <textarea id="field_6ez9tb" value={form.description} onChange={e => setForm({
              ...form,
              description: e.target.value
            })} placeholder="Detailed description..." rows={4} style={{
              width: "100%",
              padding: 8,
              borderRadius: 8,
            ...
            gap: 12
                  ...
                  <div className="input-v2" style={{
              flex: 1
            }}>
                    <label htmlFor="field_hfds17">Category</label>
                    <select id="field_hfds17" value={form.category} onChange={e => setForm({
                ...form,
              ...
              flex: 1
                    ...
                    <select id="field_tapocp" value={form.priority} onChange={e => setForm({
                ...form,
                ...
                <button className="modal-submit-btn" onClick={createTicket} disabled={submitting} style={{
            marginTop: 12
          }}>
...
function SupportTicketsSection3({
  ...
  return <AnimatePresence>
        {selectedTicket && <div className="modal-overlay-v2">
            <m.div className="modal-content-glass" initial={{
        opacity: 0,
        scale: 0.95
      }} animate={{
        ...
        overflow: "auto"
            ...
            marginBottom: 16
                ...
                {selectedTicket.resolutionSummary && <div style={{
            padding: 12,
            background: "#10b98110",
            ...
            marginBottom: 16
                ...
                {selectedTicket.status === "RESOLVED" && <div style={{
            display: "flex",
            gap: 8,
            marginBottom: 16
          }}>
                    <button className="sub-upgrade-btn" style={{
              fontSize: 12
              ...
              if (reason) reopenTicket(selectedTicket.id, reason);
                ...
                {selectedTicket.status === "CLOSED" && <button className="sub-upgrade-btn" style={{
            fontSize: 12,
            marginBottom: 16
          }} onClick={() => {
            const reason = prompt("Reason for reopening:");
            ...
            paddingTop: 16
              ...
              marginBottom: 16
                    ...
                    {(selectedTicket.replies || selectedTicket.messages || []).map(msg => <div key={msg.id} style={{
                padding: 10,
                borderRadius: 8,
                background: msg.authorRole === "ADMIN" || msg.senderRole === "ADMIN" ? "var(--primary-container)" : "var(--surface)",
                alignSelf: msg.authorRole === "ADMIN" || msg.senderRole === "ADMIN" ? "flex-end" : "flex-start",
                maxWidth: "80%"
              }}>
                        <div style={{
                  fontSize: 11,
                  ...
                  {selectedTicket.status !== "CLOSED" && <div style={{
              display: "flex",
              gap: 8
            }}>
                      <><label htmlFor="field_xjoaon" className="sr-only">Type a reply...</label><input type="text" value={replyText} onChange={e => setReplyText(e.target.value)} placeholder="Type a reply..." className="pos-input" style={{
                  flex: 1
                }} onKeyDown={e => e.key === "Enter" && sendReply()} id="field_xjoaon" /></>
...
export default function SupportTickets({
  ...
  return <div className="sub-container">
      ...
      {dashboard && <div className="sub-current-card" style={{
      marginBottom: 16
    }}>
          <div style={{
        display: "flex",
      ...
      marginBottom: 16
        ...
        <select value={filter.status} onChange={e => setFilter({
        ...filter,
        status: e.target.value
      })} className="pos-input" style={{
        width: "auto"
      }}>
        ...
        <select value={filter.priority} onChange={e => setFilter({
        ...filter,
        priority: e.target.value
      })} className="pos-input" style={{
        width: "auto"
      ...
      <SupportTicketsSection1 e={e} loadTicketDetails={loadTicketDetails} ticket={ticket} />

      <SupportTicketsSection2 setShowCreate={setShowCreate} setForm={setForm} form={form} />

      <SupportTicketsSection3 setSelectedTicket={setSelectedTicket} closeTicket={closeTicket} selectedTicket={selectedTicket} reopenTicket={reopenTicket} setReplyText={setReplyText} sendReply={sendReply} />
    </div>;
}
function Stat({
  label,
