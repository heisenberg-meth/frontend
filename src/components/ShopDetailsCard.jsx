...
function ShopModalSection1({
  ...
  return <div className="sys-modal sys-modal-wide" style={{
    textAlign: "left"
  }} onClick={e => e.stopPropagation()} role="presentation">
        <>
          <h2 style={{
        marginBottom: 24
      }}>Edit Shop & Invoice Details</h2>
          ...
          <div className="sys-form-group">
            ...
            <input id="field_90jjmb" className="sys-input" value={form.shopName} onChange={e => handleChange("shopName", e.target.value)} />
          </div>
          <div className="sys-form-group">
            <label htmlFor="field_ieqpr3" className="sys-label">
              Phone
            </label>
            <input id="field_ieqpr3" className="sys-input" value={form.phone} onChange={e => handleChange("phone", e.target.value)} />
          </div>
          <div className="sys-form-group">
            <label htmlFor="field_cpbgvo" className="sys-label">
              Email
            </label>
            <input id="field_cpbgvo" className="sys-input" value={form.email} onChange={e => handleChange("email", e.target.value)} />
          </div>
          <div className="sys-form-group">
            <label htmlFor="field_8be5xd" className="sys-label">
              Address
            </label>
            <textarea id="field_8be5xd" className="sys-input sys-textarea" rows="3" value={form.address} onChange={e => handleChange("address", e.target.value)} />
          </div>
          ...
          <div className="sys-form-group">
            ...
            <input id="field_vr4xbg" className="sys-input" value={form.invoicePrefix} onChange={e => handleChange("invoicePrefix", e.target.value)} />
          </div>
          <div className="sys-form-group">
            <label htmlFor="field_770brf" className="sys-label">
              Footer Text
            </label>
            <textarea id="field_770brf" className="sys-input sys-textarea" rows="2" value={form.footerText} onChange={e => handleChange("footerText", e.target.value)} />
          </div>
          <div className="sys-form-group">
            <label htmlFor="field_1n33rj" className="sys-label">
              Logo URL
            </label>
            <input id="field_1n33rj" className="sys-input" value={form.logoUrl} onChange={e => handleChange("logoUrl", e.target.value)} />
          </div>

          <div className="modal-toggles">
            <div className="sys-toggle-row">
              <span className="sys-toggle-label">Show Logo</span>
              <div role="button" tabIndex={0} className={`sys-toggle ${form.showLogo ? "on" : ""}`} onKeyDown={e => {
            ...
            <div className="sys-toggle-row">
              ...
              <div role="button" tabIndex={0} className={`sys-toggle ${form.showDoctorName ? "on" : ""}`} onKeyDown={e => {
            if (e.key === "Enter" || e.key === " ") {
              e.preventDefault();
              e.currentTarget.click();
            }
          }} onClick={() => handleChange("showDoctorName", !form.showDoctorName)}>
                <div className="sys-toggle-thumb" />
              </div>
            </div>
            <div className="sys-toggle-row">
              <span className="sys-toggle-label">QR Code</span>
              <div role="button" tabIndex={0} className={`sys-toggle ${form.showQRCode ? "on" : ""}`} onKeyDown={e => {
            if (e.key === "Enter" || e.key === " ") {
              e.preventDefault();
              e.currentTarget.click();
            }
            ...
            <div className="sys-toggle-row">
              ...
              <div role="button" tabIndex={0} className={`sys-toggle ${form.showHSNCode ? "on" : ""}`} onKeyDown={e => {
            if (e.key === "Enter" || e.key === " ") {
              e.preventDefault();
              e.currentTarget.click();
            }
            ...
            <div className="sys-toggle-row">
              ...
              <div role="button" tabIndex={0} className={`sys-toggle ${form.showExpiryDate ? "on" : ""}`} onKeyDown={e => {
            if (e.key === "Enter" || e.key === " ") {
              e.preventDefault();
              e.currentTarget.click();
            }
          }} onClick={() => handleChange("showExpiryDate", !form.showExpiryDate)}>
                <div className="sys-toggle-thumb" />
              </div>
            </div>
            <div className="sys-toggle-row">
              <span className="sys-toggle-label">GST Breakdown</span>
              <div role="button" tabIndex={0} className={`sys-toggle ${form.showGSTBreakdown ? "on" : ""}`} onKeyDown={e => {
            if (e.key === "Enter" || e.key === " ") {
              e.preventDefault();
              e.currentTarget.click();
            }
          }} onClick={() => handleChange("showGSTBreakdown", !form.showGSTBreakdown)}>
                <div className="sys-toggle-thumb" />
              </div>
            </div>
          </div>
          ...
          <div className="modal-actions">
            ...
            <button className="sys-btn-fill" onClick={handleSave} disabled={!hasChanges || saving}>
...
export default function ShopDetailsCard({
  ...
  return <>
      <div className="sys-card">
        ...
        <button className="sys-btn-outline" style={{
        width: "100%",
        marginTop: 16
      ...
      {modalOpen && <ShopModal onClose={() => setModalOpen(false)} onRefresh={onRefresh} showToast={showToast} storeProfile={sp} invoiceTemplate={inv} />}
    </>;
}
function ShopModal({
  onClose,
  ...
  const hasChanges = original !== null && Object.keys(form).some(k => form[k] !== original[k]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await Promise.all([api.put(API_ROUTES.SETTINGS_STORE_PROFILE, profilePayload), api.put(API_ROUTES.SETTINGS_INVOICE_TEMPLATE, invoicePayload)]);
  ...
  return <div role="button" tabIndex={0} className="sys-modal-overlay" ref={overlayRef} onKeyDown={e => {
    if (e.key === "Enter" || e.key === " ") {
      e.preventDefault();
      e.currentTarget.click();
    }
    ...
    if (e.target === overlayRef.current) onClose();
      ...
      <ShopModalSection1 handleChange={handleChange} e={e} form={form} />
    </div>;
}