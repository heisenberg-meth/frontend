...
import { AlertTriangle, Download, X, Bell, MessageCircle, Mail, Phone, ClipboardList, ChevronRight, Package, Loader2 } from "lucide-react";
...
function POModal({
  ...
  return <div role="button" tabIndex={0} className="lsa-modal-overlay" onKeyDown={e => {
    if (e.key === "Enter" || e.key === " ") {
      e.preventDefault();
      e.currentTarget.click();
    }
  }} onClick={onClose}>
      <div className="lsa-modal-box" onClick={e => e.stopPropagation()} role="presentation">
        ...
        <div className="lsa-modal-body">
          ...
          padding: 14
            ...
            {supplier.items.map(item => <div key={item.name || item.drug} style={{
            display: "flex",
          ...
          <div className="lsa-modal-field">
            ...
            <input id="field_gpm4ga" required className="lsa-modal-input" type="number" min="1" placeholder="e.g. 200" value={qty} onChange={e => setQty(e.target.value)} />
          </div>
          <div className="lsa-modal-field">
            <label htmlFor="field_jc1v6b" className="lsa-modal-label">
              Notes (optional)
            </label>
            <input id="field_jc1v6b" required className="lsa-modal-input" placeholder="Priority delivery required…" value={note} onChange={e => setNote(e.target.value)} />
          </div>
        </div>
        <div className="lsa-modal-actions">
          <button className="lsa-btn-outline" style={{
          flex: 1
        }} onClick={onClose}>
            Cancel
          </button>
          <button className="lsa-btn-primary" style={{
          flex: 1
        }} onClick={submit} disabled={saving || !qty}>
            {saving ? "Raising PO…" : <>
...
function SupplierCard({
  ...
  return <div className={`lsa-group-card ${supplier.severity}`}>
      <div className="lsa-group-header">
        <div className="lsa-group-header-left">
          <div className="lsa-supplier-icon" style={{
          background: supplier.severity === "critical" ? "rgba(239,68,68,0.1)" : "rgba(245,158,11,0.1)",
          color: supplier.severity === "critical" ? "var(--danger)" : "var(--warning)"
        }}>
        ...
        <div className="lsa-group-header-right">
          ...
          <button className="lsa-create-po-btn flex items-center gap-2" onClick={() => onCreatePO(supplier)}>
      ...
      <table className="lsa-mini-table">
        <thead>
          <tr>
            {["Drug Name", "Current Stock", "Min Threshold", "Days to Stockout", "Urgency", "Action"].map(h => <th key={h}>{h}</th>)}
          </tr>
        </thead>
        <tbody>
          {supplier.items.map(item => <tr key={item.name || item.drug}>
              <td>
                <div className="lsa-drug-name">
                  <div className="lsa-drug-dot" style={{
              ...
              <td>
                <span className={`lsa-stockout ${item.daysToStockout <= 2 ? "urgent" : ""}`}>
              ...
              <td>
                <button className="lsa-reorder-btn" onClick={() => onCreatePO(supplier)}>
                  Reorder
                </button>
              </td>
            </tr>)}
...
function NotifySettings({
  ...
  const [wa, setWa] = useState(settings?.notificationChannels?.whatsapp ?? true);
  const [sms, setSms] = useState(settings?.notificationChannels?.sms ?? false);
  const [email, setEmail] = useState(settings?.notificationChannels?.email ?? true);
  ...
  const [mail, setMail] = useState(settings?.alertEmail || "admin@viyanmedassist.in");
  ...
  return <div className="lsa-notify-card">
      ...
      <div className="lsa-notify-body">
        <div className="lsa-notify-row">
          <div className="lsa-notify-toggle-group">
            <label className="lsa-toggle-label">
              <div className="lsa-toggle">
                <input required type="checkbox" checked={wa} onChange={e => setWa(e.target.checked)} />
                <div className="lsa-toggle-slider" />
              </div>
              <div>
                <div className="lsa-toggle-text" style={{
                display: "flex",
                alignItems: "center",
                gap: 6
              }}>
          ...
          <><label htmlFor="field_a1e0zu" className="sr-only">WhatsApp phone number</label><input required className="lsa-notify-input" placeholder="WhatsApp phone number" value={phone} disabled={!wa} onChange={e => setPhone(e.target.value)} id="field_a1e0zu" /></>
        </div>
        <div className="lsa-notify-row">
          <div className="lsa-notify-toggle-group">
            <label className="lsa-toggle-label">
              <div className="lsa-toggle">
                <input required type="checkbox" checked={sms} onChange={e => setSms(e.target.checked)} />
                <div className="lsa-toggle-slider" />
              </div>
              <div>
                <div className="lsa-toggle-text" style={{
          ...
          <><label htmlFor="field_fy6jv7" className="sr-only">SMS phone number</label><input required className="lsa-notify-input" placeholder="SMS phone number" value={smsPhone} disabled={!sms} onChange={e => setSmsPhone(e.target.value)} id="field_fy6jv7" /></>
        </div>
        <div className="lsa-notify-row">
          <div className="lsa-notify-toggle-group">
            <label className="lsa-toggle-label">
              <div className="lsa-toggle">
                <input required type="checkbox" checked={email} onChange={e => setEmail(e.target.checked)} />
                <div className="lsa-toggle-slider" />
              </div>
              <div>
                <div className="lsa-toggle-text" style={{
          ...
          <><label htmlFor="field_1c14np" className="sr-only">Email address</label><input required className="lsa-notify-input" type="email" placeholder="Email address" value={mail} disabled={!email} onChange={e => setMail(e.target.value)} id="field_1c14np" /></>
          <button className="lsa-notify-save" onClick={handleSave} disabled={saving}>
            {saving ? <>
...
export default function LowStockAlerts({
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      try {
        setLoading(true);
        const [lowStockRes, settingsRes] = await Promise.all([getLowStockMedicines(), getStockAlerts()]);
        if (!mounted) return;
        const lowStockData = lowStockRes.data.data || lowStockRes.data;
        const items = Array.isArray(lowStockData) ? lowStockData : [];
        const grouped = {};
        items.forEach(item => {
          const supName = item.supplierName || item.supplier || "Unknown Supplier";
          if (!grouped[supName]) {
            grouped[supName] = {
          ...
          const urgency = currentQty === 0 || currentQty <= reorderPt / 2 ? "critical" : "low";
  ...
  const handleExport = () => {
    const rows = [["Supplier", "Drug Name", "Current Stock", "Min Threshold", "Urgency"], ...lowStockItems.flatMap(s => s.items.map(i => [s.name, i.name, i.quantity, i.reorderLevel || 10, i.urgency, i.purchaseCost || 0]))].map(r => r.map(v => `"${v}"`).join(",")).join("\n");
  ...
  const handleCreatePO = async (supplier, qty, note) => {
    ...
    try {
      ...
      const subtotal = items.reduce((acc, i) => acc + i.quantity * i.purchasePrice, 0);
  ...
  return <div className="lsa-container">
      <div className="lsa-breadcrumb">
        <button className="lsa-breadcrumb-item" onClick={() => navigate("/dashboard")}>
          Dashboard
        </button>
        <span className="lsa-breadcrumb-sep">/</span>
        <button className="lsa-breadcrumb-item" onClick={() => navigate("/stock")}>
          Inventory
        </button>
        <span className="lsa-breadcrumb-sep">/</span>
      ...
      <div className="lsa-filter-bar">
        <div className="lsa-tabs">
          ...
          badge: 0,
          cls: "expiring"
        }].map(t => <button key={t.key} className={`lsa-tab ${activeTab === t.key ? "active" : ""}`} onClick={() => setTab(t.key)}>
              {t.label}
      ...
      color: "var(--text-dim)"
        ...
        </div> : filteredSuppliers.length === 0 ? <div className="empty-state" style={{
      textAlign: "center",
      padding: "100px 0",
      ...
      border: "1px solid var(--outline-variant)"
        ...
        </div> : <div className="lsa-groups">
          {filteredSuppliers.map(supplier => <SupplierCard key={supplier.name} supplier={supplier} onCreatePO={s => setPO(s)} />)}
        </div>}
      ...
      marginTop: 12
        ...
        <button className="lsa-btn-outline" onClick={() => navigate("/purchases")}>
          View Active Purchase Orders <ChevronRight size={14} />
        </button>
      </div>

      {poSupplier && <POModal supplier={poSupplier} onClose={() => setPO(null)} onSuccess={handleCreatePO} saving={poSaving} />}
    </div>;
}