...
const PLANS = [{
  ...
  highlight: false,
  features: ["28 Days Free Access", "Full Platform Access", "Inventory & Batch Tracking", "Billing & POS System", "Reports & Analytics", "Barcode & QR Scanning", "Email Support"]
}, {
  id: "basic-monthly",
  name: "Premium",
  price: {
  ...
  features: ["Everything in Free Trial", "Unlimited Medicines & Batches", "Unlimited GST Billing & POS", "Cloud Backup & Real-Time Sync", "Supplier & Expiry Intelligence", "Priority WhatsApp & Phone Support", "Regular System Updates"]
}];
function SubscriptionCRUDSection1({
  initiateUpgrade
}) {
  return <div className="sub-current-card">
        ...
        color: "var(--primary, #10b981)"
          ...
          <div className="sub-current-info">
            {subscription?.status === "ACTIVE" ? <>
                <div className="ent-label" style={{
            fontSize: "1.3rem",
            fontWeight: "800"
          }}>
                  {subscription.planName || "MedAssist Active Plan"}
                </div>
                <div className="ent-price" style={{
          ...
          {(isTrial || isPending || isExpired) && <button className="sub-upgrade-btn" onClick={() => initiateUpgrade(PLANS.find(p => p.id === "basic-monthly") || PLANS[1])}>
              <Sparkles size={16} /> Upgrade to Premium
            </button>}
        </div>
      </div>;
}
export default function SubscriptionCRUD({
  showToast,
  user
}) {
  const {
  ...
  const refreshSubscriptionData = useCallback(async () => {
    try {
      const [subRes, payRes] = await Promise.all([api.get(API_ROUTES.SUBSCRIPTIONS_STATUS), api.get(API_ROUTES.PAYMENTS_HISTORY)]);
      ...
      const options = {
        ...
        handler: async function (rzpResponse) {
          try {
            ...
            pollRef.current = setInterval(async () => {
              ...
              try {
                ...
                if (res.data?.paymentStatus === "SUCCESS") {
                  ...
                  clearInterval(pollRef.current);
                  showToast("Payment verification timed out. If money was deducted, it will be refunded or manually activated.", "error");
                  setUpgrading(false);
                }
              } catch (e) {
                if (attempts > 15) {
            ...
            showToast("Payment received but verification failed. Our team has been notified. Please do not make another payment.", "error");
            setUpgrading(false);
          }
        },
        theme: {
  ...
  return <div className="sub-container">
      ...
      <SubscriptionCRUDSection1 initiateUpgrade={initiateUpgrade} />
      ...
      <div className="sub-plans-section">
        ...
        <div className="sub-plans-grid">
          {PLANS.map(plan => {
          const isCurrentPlan = plan.id === "free-trial" && isTrial || plan.id === subscription?.planId || subscription?.planName && plan.name.toLowerCase() === subscription.planName.toLowerCase();
          return <div key={plan.id} className={`sub-plan-card ${plan.highlight ? "sub-plan-card--highlight" : ""} ${isCurrentPlan ? "sub-plan-card--current" : ""}`}>
                {plan.popularText && <div className="sub-plan-popular">{plan.popularText}</div>}
                {plan.badgeText && <div className="sub-plan-badge-top">{plan.badgeText}</div>}
                <div className="sub-plan-name">{plan.name}</div>
                <div className="sub-plan-sub-desc">{plan.sub}</div>
                <div className="sub-plan-price">
                  ₹
                  {billingCycle === "monthly" ? plan.price.monthly : plan.price.annual}
                  <span className={plan.duration.startsWith("/") ? "sub-plan-dur" : "sub-plan-dur-badge"}>
                    {plan.duration}
                  </span>
                </div>
                <ul className="sub-plan-features">
                  {plan.features.map((f, idx) => <li key={`${plan.id}-feat-${idx}`}>
                      <span className="sub-check">✓</span>
                      <span>{f}</span>
                    </li>)}
                </ul>
                <button className={`sub-plan-btn ${plan.highlight ? "btn-highlight" : "btn-outline"} ${isCurrentPlan || plan.id === "free-trial" ? "disabled" : ""}`} onClick={() => !isCurrentPlan && plan.id !== "free-trial" && initiateUpgrade(plan)} disabled={isCurrentPlan || plan.id === "free-trial"}>
                  {isCurrentPlan ? "Current Active Plan ✓" : plan.id === "free-trial" ? "Trial Access" : `Upgrade to ${plan.name}`}
                </button>
              </div>;
        })}
        </div>
      ...
      <div className="sub-payment-section">
        ...
        <table className="sub-payment-table">
          ...
          <tbody>
            {paymentHistory.map(pay => <tr key={pay.id}>
                <td>{pay.id}</td>
                <td>
